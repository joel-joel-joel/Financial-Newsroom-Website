 ...
 * NewsAPI doesn't provide direct article ID lookup, so we search by topic
 */
async function fetchArticleFromAPI(articleId) {
    try {
        console.log('Fetching article from API...');

        // General search for demo purposes
        ...
        if (results.articles && results.articles.length > 0) {
            ...
            return results.articles[0]; // Return first relevant article
        }
...
async function loadRelatedArticles() {
    try {
        ...
        // Related articles are optional, do not throw
    }
}
...
// Make goBack globally accessible for back button
